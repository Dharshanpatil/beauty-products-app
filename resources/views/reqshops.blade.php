            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-wrap mb-3">
                        <h2 class="page-title">Shops Requsts</h2>
                    </div>
...
<td><div class="image-fit h-24 w-24 lg:mr-1 lg:h-12 lg:w-12">
     <img class="avatar avatar-lg mr-3 avatar--3-1 rounded-full" src="/storage/app/public/docs/{{$user['document']}}">
     </div>
     </td>
                                                                                                        <td>{{$user['email']}}</td>
                                            ...
                                            <button type="button"
                                                                    @if(env('APP_ENV')!='demo')
                                                                    onclick="form_alert('approve-{{$user['id']}}','want to appeove this shop?')"
                                                                    @endif
                                                                    class="table-actions_delete bg-transparent border-0 p-0 demo_check">
                                                                <span class="material-icons">approval</span>
                                                            </button>
                                                             <form action="{{route('admin.shop-approve',$user['id'])}}"
                                                                  method="post" id="approve-{{$user['id']}}" class="hidden">
                                                                @csrf
